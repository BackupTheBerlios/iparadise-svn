...
function setColor(color, colorTextProperty) {
    var link = getObj(curId);
    var field = getObj('bgColor');
    ...
    eval(getObj(colorTextProperty).title);
}

function setDiv(colorTextProperty) {
...
function pickColor(id, colorTextProperty) {
    if (!divSet) {
        setDiv(colorTextProperty);
    }
    var picker = getObj('colorpicker');
...
function getColorTable(colorTextProperty) {
    ...
    for (i = 0; i < colors.length; i++) {
        ...
        tableCode += '<td bgcolor="#000000"><a style="outline: 1px solid #000000; color: '
                ...
                + colors[i] + '" href="javascript:setColor(\'' + colors[i] + '\', \'' + colorTextProperty + '\');">$nsb</a></td>';
...
function relateColor(id, color, colorTextProperty) {
    var link = getObj(id);
    if (color == '') {
        link.style.background = nocolor;
        link.style.color = nocolor;
